...
// camera
Camera camera(glm::vec3(0.0f, -1.0f, 0.0f)); //overview camera
//Camera camera(glm::vec3(0.0, 0.0f,3.0f));  //1 person camera
...
int main()
{
	...
	//bool res = loadOBJ("../Models/Sam.obj", teapotVertices, teapotTextures, teapotNormals);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		modelTeapot = glm::scale(modelTeapot, glm::vec3(0.2f, 0.2f, 0.2f));
		...
		if (useQuaternion) {
			...
			if (pressYaw) {
				...
				//glm::quat orientationYAW = qPitch * qYaw;
				//orientation = glm::normalize(orientation);
				//glm::mat4 rotateYAW = glm::mat4_cast(orientationYAW);  

				//glm::mat4 translate = glm::mat4(1.0f);
				//translate = glm::translate(translate, -eye);

				//modelTeapot = rotateYAW;// *translate;
			}
			//-------------------------------------------------------------	ROLL
			if (pressRoll) {
				orientation = qPitch * qRoll;
				//glm::quat orientationROLL = qPitch * qRoll;
				//glm::mat4 rotateROLL = glm::mat4_cast(orientationROLL);  
				//modelTeapot = rotateROLL; 
			}
			//------------------------------------------------------------- PITCH 
			if (pressPitch) {
				orientation = qYaw * qRoll;
				//glm::quat orientationPITCH = qYaw * qRoll;
				//glm::mat4 rotatePITCH = glm::mat4_cast(orientationPITCH);  
				//modelTeapot = rotatePITCH;
			}
			...
			translate = glm::translate(translate, glm::vec3(0.0f, 0.0f, -10.0f));
			...
			modelTeapot = translate * rotate * rotate2;
			//modelTeapot = translate * rotate;
			//modelTeapot = rotate;
		...
		glm::mat4 mvpTeapot = projection_matrix * viewTeapot * modelTeapot;
		//glm::vec3 lightPos = glm::vec3(-2, 4, -4);
		//glm::vec3 lightPos = glm::vec3(-6, 1, -3);
		...
		modelpropeller = modelTeapot;
		
		modelpropeller = glm::scale(modelpropeller, glm::vec3(1.5f, 1.5f, 1.5f));
		modelpropeller = glm::translate(modelpropeller, glm::vec3(2.1f, 0.5f, 0.0f));
		...
		//glm::vec3 skew;
		//glm::vec4 persp;
		//glm::decompose(modelTeapot, scal, orient, trans, skew, persp);
		/// ---------------------
		...
		glm::vec3 skewv;
		glm::vec4 perspv;
		glm::decompose(view, scalv, orientv, transv, skewv, perspv);*/
		...
		/*glm::mat3 rotv = glm::mat3_cast(orient);
		glm::mat3 vie = glm::mat3_cast(view);*/


		///view = glm::mat4(glm::mat3(camera.GetViewMatrix()));
		//view = glm::lookAt(glm::vec3(0.0f, -1.0f, 0.2f), glm::vec3(0.0f, 0.0f, 0.0f), vec3::glm(0.0f, 1.0f, 0.0f));
		...
		skyboxShader.setMat4("viewsky", viewTeapot);
		//skyboxShader.setMat4("viewsky", viewtemp);
